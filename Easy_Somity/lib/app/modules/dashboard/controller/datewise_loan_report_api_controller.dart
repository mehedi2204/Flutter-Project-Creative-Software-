...
import '../../loan report/datewise_loan_report_model.dart';
import '../../signin/loginController.dart';



class DatewiseLoanReportController extends GetxController {
  ...
  Future<void> fetchInitialData() async {
    ...
    final response = await http.get(
      Uri.parse(
          'https://softwaredemo.shop/Easy_Somity/api/date-wise-loan-report?form_date=$formattedFromDate&to_date=$formattedToDate'),
      headers: {
    ...
    if (response.statusCode == 201) {
      ...
      final report = datewiseLoanReport.fromJson(jsonResponse); // Use the new model here
  ...
  Future<void> fetchMemberData() async {
    ...
    final response = await http.get(
      Uri.parse(
          'https://softwaredemo.shop/Easy_Somity/api/date-wise-loan-report?form_date=$fromDate&to_date=$toDate'),
      headers: {
        'Authorization': 'Bearer ${_authController.user.value.accessToken}',
      },
    );
