...
class LoanInvoiceViewPOSprint extends StatelessWidget {
  const LoanInvoiceViewPOSprint({Key? key}) : super(key: key);
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text("Loan Collection Invoice Print",
                            style: TextStyle(
                                fontSize:
...
class _PrinterSelectState extends State<PrinterSelect> {
  ...
  _initBluetooth() async {
    try {
      ...
      customSnakebar(
          title: 'Error initializing Bluetooth',
          description: '',
  ...
  Future<void> _printInvoice() async {
    try {
      ...
      if (_writeCharacteristic != null && controller.isLoading.value == false) {
        ...
        invoiceMessage += "Loan Collection Number:${controller.data.value.installmentCollectionsNo ?? ''}\n\n";
        invoiceMessage += "Installment Amount:${controller.amountPerInstallment.value.installmentAmount ?? ''}\n";
        invoiceMessage += "Loan Amount:${_loanCollectionController.loanCollectionData.value.data!.totalAmount}\n";
        invoiceMessage += "Today's Collection:${controller.data.value.installmentCollection ?? '0.0'}\n";
        invoiceMessage += "Total Collection:${_loanCollectionController.loanCollectionData.value.totalCollection ?? '0.0'}\n";
        invoiceMessage += "Balance:${totalAmount-totalCollectionParsed}\n";
        invoiceMessage += "Previous Due:${controller.data.value.previousDue ?? '0.0'}\n";
        invoiceMessage += "Fine:${controller.data.value.fine ?? '0.0'}\n";
        invoiceMessage += "Note:${controller.data.value.note ?? ''}\n\n\n\n";
  ...
  Future<void> _disconnectFromDevice() async {
    try {
      ...
      // Fluttertoast.showToast(
      //   msg: "Error disconnecting from the printer: $ex",
      //   toastLength: Toast.LENGTH_LONG,
      //   gravity: ToastGravity.CENTER,
      //   backgroundColor: Colors.red,
      //   textColor: Colors.white,
